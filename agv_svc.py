...
class Danbach_AGV():
    ...
    def forward(self, distance, speed=Danbach_AGV.DEFAULT_SPEED):
        if distance == 0 or speed == 0:
            return
        l0, r0 = self.__get_wheel_odo__()
        t_0 = time.time()-Danbach_AGV.CMD_PERIOD
        while True:
            l, r = self.__get_wheel_odo__()
            if l >= l0+distance and r >= r0+distance:
                break
            if time.time()-t0 > Danbach_AGV.CMD_PERIOD:
                t0 = time.time()
                self.__set_wheel__(speed, speed)

        self.__set_wheel__(0, 0)

    def back(self, distance, speed=Danbach_AGV.DEFAULT_SPEED):
        if distance == 0 or speed == 0:
            return
        l0, r0 = self.__get_wheel_odo__()
        t_0 = time.time()-Danbach_AGV.CMD_PERIOD
    ...
    def pivot(self, radian, speed=Danbach_AGV.DEFAULT_SPEED):
        if radian == 0 or speed == 0:
            return
        l0, r0 = self.__get_wheel_odo__()
        dist = Danbach_AGV.WHEEL_DIST/2.0 * radian
        t_0 = time.time()-Danbach_AGV.CMD_PERIOD
        while True:
            l, r = self.__get_wheel_odo__()
    ...
    def steer(self, radian, direction=1, speed=Danbach_AGV.DEFAULT_SPEED):
        if radian == 0 or speed == 0:
            return
        l0, r0 = self.__get_wheel_odo__()
        dist = Danbach_AGV.WHEEL_DIST * radian
        t_0 = time.time()-Danbach_AGV.CMD_PERIOD
        while True:
            l, r = self.__get_wheel_odo__()
    ...
    def turn(self, radian, inner_radius, direction=1, speed=Danbach_AGV.DEFAULT_SPEED):
        pass
